...
class Statistics(object):
    def __init__(self):
        self.total_actions_hists = {}

    def add_data(self, data):
        action_to_letter = {}
        for action in data:
            if action[0] != 'replace':
                action_to_letter.setdefault(action[0],[]).append(action[1])
            else:
                action_to_letter.setdefault('replace1', []).append(action[1])
                action_to_letter.setdefault('replace2', []).append(action[2])
        for action, letters in action_to_letter.items():
            letter, count = np.unique(letters, return_counts=True)
            self.total_actions_hists[action] = dict(Counter(self.total_actions_hists.setdefault(action, {})) + Counter(dict(zip(letter, count))))
    def get_interesting_statistics(self):
        ec_per_letter = dict(Counter(self.total_actions_hists['replace1']) +
                             Counter(self.total_actions_hists['replace2']) +
                             Counter(self.total_actions_hists['insert']) +
                             Counter(self.total_actions_hists['delete']))
        self.total_actions_hists['total_errors'] = ec_per_letter

        for ec_dict in self.total_actions_hists.items():
            er_per_letter = {}
            for letter, ec in ec_dict.items():
                er_per_letter[letter] = ec / (self.total_actions_hists['equal'].setdefault(letter, 0) + ec)
...
class SequenceMatcher(object):
    ...
    def _compute_distance_fast(self):
        ...
        d, m = my_edit_distance(self.seq1, self.seq2,
                                action_function=self.action_function,
                                test=self.test)
        if self.dist:
            assert d == self.dist
...
def my_edit_distance(seq1, seq2, action_function=lowest_cost_action, test=operator.eq):
...
def my_edit_distance_backpointer(seq1, seq2, action_function=lowest_cost_action, test=operator.eq):
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            ...
            if action == 'equal':
                ...
                bp[i][j] = ['equal', seq1[i-1]]
            elif action == 'replace':
                d[i][j] = sub_cost
                matches[i][j] = sub_match
                bp[i][j] = ['replace', seq1[i - 1], seq2[j - 1]]
            elif action == 'insert':
                d[i][j] = ins_cost
                matches[i][j] = ins_match
                bp[i][j] = ['insert', seq2[j-1]]
            elif action == 'delete':
                d[i][j] = del_cost
                matches[i][j] = del_match
                bp[i][j] = ['delete', seq1[i-1]]
            else:
...
def edit_distance_backpointer(seq1, seq2, action_function=lowest_cost_action, test=operator.eq):
    ...
    for i in range(1, m + 1):
        d[i][0] = i
        bp[i][0] = [('delete', seq1[i-1]), i - 1, i, 0, 0]
    # target prefixes can be reached from empty source prefix by inserting
    # every characters
    for j in range(1, n + 1):
        d[0][j] = j
        bp[0][j] = [('insert', seq2[j-1]), 0, 0, j - 1, j]
    # compute the edit distance...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            ...
            if action == 'equal':
                d[i][j] = sub_cost
                matches[i][j] = sub_match
                bp[i][j] = [('equal', seq1[i-1]), i - 1, i, j - 1, j]
            elif action == 'replace':
                d[i][j] = sub_cost
                matches[i][j] = sub_match
                bp[i][j] = [('replace', seq1[i - 1], seq2[j - 1]), i - 1, i, j - 1, j]
            elif action == 'insert':
                d[i][j] = ins_cost
                matches[i][j] = ins_match
                bp[i][j] = [('insert', seq2[j-1]), i - 1, i - 1, j - 1, j]
            elif action == 'delete':
                d[i][j] = del_cost
                matches[i][j] = del_match
                bp[i][j] = [('delete', seq1[i-1]), i - 1, i, j - 1, j - 1]
            else:
                raise Exception('Invalid dynamic programming action returned!')
