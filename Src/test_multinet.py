...
from dataset.data_transform_simplex import Resize, AddWidth, Normalize
...
def test(nets, data, abc, cuda, visualize, batch_size=1,
    ...
    for i, sample in enumerate(iterator):
        ...
        for neti, net in enumerate(nets):
            if cuda:
                imgs = imgs.cuda(neti)
            orig_seq = net(imgs, img_seq_lens, decode=False, do_beam_search=do_beam_search)
        ...
        all_nets_out, _ = stats.mode(all_nets_out, axis=0, )
